...
HEATER_OFF_THRESHOLD = 97.5  # Threshold for turning off the heater depending the SOC
HEATER_ON_THRESHOLD = 99.75   # Threshold for turning on the heater
...
while True:
    ...
    if valid_data == 1:
        ...
        # Check if the heater is already off
        if (heater == HEATER_OFF) and (previous_SOC > HEATER_OFF_THRESHOLD) and (SOC <= HEATER_OFF_THRESHOLD) and not is_turned_off:
            logging.info('Heater is already OFF. No action needed.')
            is_turned_off = True
        ...
        elif (heater == HEATER_ON) and (previous_SOC < HEATER_ON_THRESHOLD) and (SOC >= HEATER_ON_THRESHOLD) and not is_turned_on:
            logging.info('Heater is already ON. No action needed.')
            is_turned_on = True
        ...
        # Check if the heater needs to be turned off
        elif (heater == HEATER_ON) and (SOC <= HEATER_OFF_THRESHOLD) and not is_turned_off:
            heater = HEATER_OFF
        ...
        # Check if the heater needs to be turned on
        elif (heater == HEATER_OFF) and (SOC >= HEATER_ON_THRESHOLD) and not is_turned_on:
            heater = HEATER_ON
